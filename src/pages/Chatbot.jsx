...
const Chatbot = ({ embedded = false }) => {
  ...
  return (
    <div className={`flex flex-col w-full h-full ${embedded ? 'p-0' : 'max-w-4xl mx-auto p-4'}`}>
      {!embedded && (
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-white mb-2">Trợ lý AI Tự động</h2>
          <p className="text-blue-200 dark:text-cyan-500/80">Hỏi đáp nhanh các quy định về BHXH, BHYT, BHTN.</p>
        </div>
      )}
      ...
      <div className={`flex flex-col flex-1 overflow-hidden ${!embedded ? 'bg-blue-800/30 dark:bg-[#131b2f]/80 border border-blue-500 dark:border-slate-700/50 rounded-2xl backdrop-blur-md shadow-inner' : ''}`}>
        {/* Chat History */}
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 custom-scrollbar">
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex gap-3 max-w-[85%] ${msg.role === 'user' ? 'self-end' : 'self-start'}`}>
              {msg.role === 'model' && (
                <div className="w-8 h-8 rounded-full bg-blue-900/50 dark:bg-[#1e293b] border border-blue-400 dark:border-cyan-500/30 flex items-center justify-center text-blue-100 dark:text-cyan-400 shrink-0">
                  <Bot size={16} />
                </div>
              )}
              
              <div className={`p-3 rounded-2xl ${msg.role === 'user' 
                ? 'bg-blue-600 text-white rounded-tr-sm shadow-sm border border-blue-500' 
                : 'bg-blue-900/50 dark:bg-[#1e293b] border border-blue-500 dark:border-slate-700/50 text-blue-50 dark:text-slate-300 rounded-tl-sm'
              }`}>
                {msg.role === 'user' ? (
          ...
          {isLoading && (
            <div className="flex gap-3 self-start">
               <div className="w-8 h-8 rounded-full bg-blue-900/50 dark:bg-[#1e293b] border border-blue-400 dark:border-cyan-500/30 flex items-center justify-center text-blue-100 dark:text-cyan-400 animate-pulse">
                <Bot size={16} />
              </div>
              <div className="bg-blue-900/50 dark:bg-[#1e293b] border border-blue-500 dark:border-slate-700/50 p-3 rounded-2xl rounded-tl-sm flex items-center gap-2">
                <Loader2 className="animate-spin text-blue-200 dark:text-cyan-400" size={16} />
                <span className="text-blue-100 dark:text-slate-400 text-sm">Đang phân tích dữ liệu...</span>
              </div>
            </div>
          )}
        ...
        {/* Input form */}
      <div className="p-3 border-t border-blue-500 dark:border-slate-700/50 bg-blue-800/50 dark:bg-[#131b2f]">
        <form onSubmit={handleSend} className="flex gap-3">
          <input 
            type="text" 
            className="flex-1 bg-blue-900/50 dark:bg-[#1e293b] border border-blue-400 dark:border-slate-700/50 text-white text-sm rounded-full px-5 py-3 focus:outline-none focus:border-blue-300 dark:focus:border-cyan-500/50 focus:bg-blue-800 dark:focus:bg-[#233147] transition-all placeholder-blue-300/50 dark:placeholder-slate-500" 
